...
/**
 * Used to output some duration into a friendly string And convert x seconds
 * into X0 days/ X1 hours/ X2 minutes/ X3 seconds
 * 
...
public class TimeConverter {
	...
	public static void loadString(Resources r) {
		...
		if (units == null || unit == null || units.length != unit.length
				|| unit.length != NB_UNIT)
			throw new IllegalStateException(
					"unit or units in strings.xml is invalid");
	}

	/**
	 * Return the string from unit[tabindex] or units[tabindex] if the quantitiy
	 * is 1 or >1. Return an empty string if the quantity is 0.
	 * 
	 * @param quantity
	 * @param tabIndex
	 * @return
	...
	private static String getUnit(int quantity, int tabIndex) {
		if (tabIndex < 0 || tabIndex > unit.length
				|| unit.length != units.length)
			throw new IndexOutOfBoundsException();

		return (quantity == 0 ? "" : quantity + " "
				+ (quantity > 1 ? units[tabIndex] : unit[tabIndex]));

	}

	/**
	 * See the examples above: tab={1,2,3} then return
	 * "1 hour 2 minutes 3 seconds" tab={1,2,0} then return "1 hour 2 minutes"
	...
	public static String time(int numberOfSeconds) {
		...
		return format(numberOfDays, numberOfHours, numberOfMinutes,
				numberOfSeconds);

	}

	public static boolean isPlurial(int t) {
		return getNumberOfDays(t) > 1 || getNumberOfHours(t) > 1
				|| getNumberOfMinutes(t) > 1 || getNumberOfSeconds(t) > 1;
	}
}
